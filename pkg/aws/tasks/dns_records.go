...
func collectRecords(ctx context.Context, payload CollectRecordsPayload) error {
	...
	defer func() {
		...
		key := metrics.Key(TaskCollectRecords, payload.AccountID, payload.Region, payload.HostedZoneID)
	...
	for paginator.HasMorePages() {
		...
		if err != nil {
			logger.Error(
				"could not list resource dns records",
				"region", payload.Region,
				"account_id", payload.AccountID,
				"hosted_zone_id", payload.HostedZoneID,
				"reason", err,
	...
	records := make([]models.ResourceRecord, 0, len(recordSets))
	for _, set := range recordSets {
		for _, rr := range set.ResourceRecords {
			...
			record := models.ResourceRecord{
				...
				HostedZoneID:   payload.HostedZoneID,
				Name:           ptr.StringFromPointer(set.Name),
				IsAlias:        isAlias,
